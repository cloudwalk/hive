...
func callContractTest(t *TestEnv) {
	...
	contract, err := testcontract.NewContract(predeployedContractAddr, t.Eth)
	if err != nil {
	...
	expected, _ := new(big.Int).SetString("0x4D2", 0)
	if expected.Cmp(value) != 0 {
		t.Fatalf("UI variable has invalid value, want %d, got %d", expected, value)
	}

	expected.SetString("0x1", 0)
	//contract.AddToMap(&bind.TransactOpts{}, predeployedContractWithAddress, expected)
